...
export class Dispatcher {
  ...
  /**
   * Per-account live AbortControllers — one per in-flight worker. Used
   * by `stop_agent` (via the `account_stopped` system event) to kill
   * running Claude sessions immediately instead of waiting for them to
   * finish naturally. Without this, a stop_agent issued while a long
   * tool call is mid-flight (e.g. reading 50 emails, calling another
   * agent) would block FUTURE wakes but let the current one run to
   * completion — which is what "they kept on working" looked like
   * before this fix.
   */
  ...
  async handleEvent(account: AgenticMailAccount, event: SSEEvent): Promise<void> {
    ...
    if (event.type === 'new' && typeof event.uid === 'number') {
      ...
      // just refuse to spawn a Claude turn. This is the
  ...
  private async handleBridgeMail(account: AgenticMailAccount, event: SSEEvent): Promise<void> {
    ...
    try {
      ...
      if (route.action === 'skip-untrusted') {
        ...
        // resume the operator's bypassPermissions session on attacker-
        // controlled mail. The message is already in the inbox; the operator
        // surfaces it themselves on their next keystroke.
  ...
  private async spawnWorker(account: AgenticMailAccount, prompt: string, ctx: { kind: string; uid?: number; taskId?: string; subject?: string; from?: string }): Promise<void> {
    ...
    try {
      ...
      const set = this.activeAborts.get(account.id);
      if (set) {
        set.delete(workerAbort);
        if (set.size === 0) this.activeAborts.delete(account.id);
      }
