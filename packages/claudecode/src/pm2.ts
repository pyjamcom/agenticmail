 ...
 *
 * Failure mode: if PM2 isn't installed, `agenticmail claudecode install`
 * still writes ~/.claude.json + the subagent .md files. We just skip the
 * dispatcher step and warn the user. They can install PM2 later and run
 * `agenticmail claudecode install` again to add the entry.
 */

import { execFileSync, spawnSync } from 'node:child_process';
import { existsSync } from 'node:fs';

export const DISPATCHER_PM2_NAME = 'agenticmail-claudecode-dispatcher';
